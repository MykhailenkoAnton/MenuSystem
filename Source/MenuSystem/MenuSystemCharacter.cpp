...
AMenuSystemCharacter::AMenuSystemCharacter() : CreateSessionCompleteDelegate(FOnCreateSessionCompleteDelegate::CreateUObject(this, &AMenuSystemCharacter::OnCreateSessionComplete)),
												FindSessionsCompleteDelegate(FOnFindSessionsCompleteDelegate::CreateUObject(this, &AMenuSystemCharacter::OnFindSessionComplene)),
...
void AMenuSystemCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	//PlayerInputComponent->BindAction("CreateSessionFromC", IE_Pressed, this, &AMenuSystemCharacter::CreateGameSession);
	//PlayerInputComponent->BindAction("JoinSessionFromC", IE_Pressed, this, &AMenuSystemCharacter::JoinGameSession);
}

void AMenuSystemCharacter::CreateGameSession()
{
	//calling when pressing 1
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		...
		if (World)
		{
			//C:/Users/Mykha/Documents/Unreal Projects/Udemy/MultiplayerCourse/MenuSystem/Content/ThirdPerson/Maps/ThirdPersonMap.umap
	...
	else
	{
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Red, FString(TEXT("Failde to create session")));
		}
	}
}
...
void AMenuSystemCharacter::JoinGameSession()
{
	...
	if (!OnlineSessionInterface.IsValid())
	{
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Red, FString(TEXT("OnlineSessionInterface not valid")));
		return;
	}
...
void AMenuSystemCharacter::OnFindSessionComplene(bool bWasSuccessful)
{
	if (!OnlineSessionInterface.IsValid())
	{
		return;
	...
	for (auto Result : SessionSearch->SearchResults)
	{
		...
		if (MatchType == FString("FreeForAll"))
		{
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Cyan, FString::Printf(TEXT("Joing Match Type: %s"), *MatchType));
			}
